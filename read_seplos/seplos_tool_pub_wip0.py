#!/usr/bin/env python3
"""
seplos_tool_pub_wip0.py — Read/write all Seplos BMS 3.0 Modbus RTU registers.

Usage:
  python seplos_tool_pub_wip0.py              → all blocks
  python seplos_tool_pub_wip0.py pia          → PIA: pack info (voltage, current, SoC…)
  python seplos_tool_pub_wip0.py pib          → PIB: cell voltages + temperatures
  python seplos_tool_pub_wip0.py spa          → SPA: protection thresholds (R/W)
  python seplos_tool_pub_wip0.py sfa          → SFA: function coils on/off (R/W)
  python seplos_tool_pub_wip0.py sca          → SCA: system control (R/W)
  python seplos_tool_pub_wip0.py via          → VIA: version + serial number
  python seplos_tool_pub_wip0.py write 0x1366 30       → write register value (decimal)
  python seplos_tool_pub_wip0.py write 0x1366 0x1E     → write register value (hex)
  python seplos_tool_pub_wip0.py write_coil 0x1400 1   → set coil ON (1) or OFF (0)
"""
...
def print_block(ser: serial.Serial, block: dict):
    ...
    if is_write_only:
        ...
        print()
        print(f"  Usage: python seplos_tool_pub_wip0.py write <address_hex> <value>")
        return
...
def cmd_write(ser: serial.Serial, args: list):
    if len(args) < 2:
        print("Usage: python seplos_tool_pub_wip0.py write <address_hex> <value>")
...
def cmd_write_coil(ser: serial.Serial, args: list):
    if len(args) < 2:
        print("Usage: python seplos_tool_pub_wip0.py write_coil <address_hex> <0|1>")
